...
{
    public record GetCustomerRequestDto(int CurrentPage, int PageSize, string? Code, string? CreatedDate, string? ExpiredDate, CustomizeRequestStatus? Status);
    ...
    internal class GetCustomerCustomizeRequestQueryHandler : IRequestHandler<GetCustomerCustomizeRequestQuery, PagingResponseDto<CustomizeRequest>>
    {
        ...
        public async Task<PagingResponseDto<CustomizeRequest>> Handle(GetCustomerCustomizeRequestQuery request, CancellationToken cancellationToken)
        {
            ...
            getCustomerRequestDto.Deconstruct(out int currentPage, out int pageSize, out string? code, out string? createdDate, out string? expiredDate, out CustomizeRequestStatus? status);
