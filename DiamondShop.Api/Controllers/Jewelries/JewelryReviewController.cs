...
namespace DiamondShop.Api.Controllers.Jewelries
{
    ...
    public class JewelryReviewController : ApiControllerBase
    {
        ...
        public async Task<ActionResult> CreateJewelryReview(CreateJewelryReviewCommand createJewelryReviewCommand)
        {
            var result = await _sender.Send(createJewelryReviewCommand);
            if (result.IsSuccess)
            {
