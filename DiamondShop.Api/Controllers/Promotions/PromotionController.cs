...
namespace DiamondShop.Api.Controllers.Promotions
{
    ...
    public class PromotionController : ApiControllerBase
    {
        ...
        {
            var result = await _sender.Send(new UpdatePromotionCommand(promotionId,new UpdatePromotionInformationCommand(promotionId,request.isExcludeQualifierProduct,request.name,request.description, request.PromoCode, request.UpdateStartEndDate), request.requirements,request.gifts,request.removedRequirements,request.removedGifts));
            if (result.IsSuccess)
            {
                var mappedResult = _mapper.Map<PromotionDto>(result.Value);
                return Ok(mappedResult);
